...
layout: post
author: Balázs Pál
title : Intalling and running GADGET-2 on Linux - 2020
...
```
Omit the `-prefix` switch to install FFTW to the default location. Here the `\` marker means, this code is actually a single line, but it would overflow the margin of the page without splitting it into two rows. Also, MPI is needed to use the `--enable-mpi` switch during the configuration, that's one of the reasons, why that should be installed first.
